...
function applyBC!(global_stiff, global_jacob, global_resid, op)
    # apply Neumann BC
    Bmat = spzeros(Float64, length(op.Neumann_BC), size(global_stiff, 2))
    ...
    for i=eachindex(op.Neumann_BC)
        ...
        for iElem=1:op.mesh.numElem
            ...
            if cdof_neu[1] in curNodes
                ...
                global_resid[2op.mesh.numBasis+i] = op.Neumann_BC[i].op_val
            end
        end
    end
...
function integrate!(global_stiff, global_jacob, external, x0, fx, op)
    ...
    for iElem = 1:op.mesh.numElem
        ...
        for iGauss = 1:length(op.gauss_rule.nodes)
            ...
            global_stiff[curNodes, curNodes] += Jac_ref_par * Jac_par_phys * op.EA * (dR*dR') * op.gauss_rule.weights[iGauss]
            global_stiff[curNodes, curNodes.+off] += 0.5 * Jac_ref_par * Jac_par_phys * op.EA * (dw0dx) * (dR*dR') * op.gauss_rule.weights[iGauss]
            global_stiff[curNodes.+off, curNodes.+off] += Jac_ref_par * Jac_par_phys^2 * op.EI * (ddR*ddR') * op.gauss_rule.weights[iGauss]
            global_stiff[curNodes.+off, curNodes.+off] += 0.5 * Jac_ref_par * Jac_par_phys * op.EA * (du0dx + dw0dx^2) * (dR*dR') * op.gauss_rule.weights[iGauss]
            
            # only different entry of Jacobian
            global_jacob[curNodes.+off, curNodes.+off] += Jac_ref_par * Jac_par_phys * op.EA * (dw0dx^2) * (dR*dR') * op.gauss_rule.weights[iGauss]
        ...
        external[curNodes.+op.mesh.numBasis] += localy
    end
